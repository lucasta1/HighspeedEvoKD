...
# ---------- プロジェクト依存 ----------
from dataset.cifar10 import CIFAR10GPUData
...
def full_kd_pair(
    ...
    # --- 学習ループ ---
    data_num = len(train_images)  # データセットの長さを取得
    for epoch in range(args.full_kd_epochs):
        student_model.train()
        indices = torch.randperm(data_num, device=device)
        for i in range(0, data_num, args.batch):
            optimizer.zero_grad(set_to_none=True)
            idx = indices[i:i + args.batch]
            ...
            loss = (
                args.kd_weight * kd_loss_fn(
                    F.log_softmax(student_outputs / 4, dim=1),
                    F.softmax(teacher_outputs / 4, dim=1)
                )
                + args.ce_weight * ce_loss_fn(student_outputs, labels)
            )
    ...
    # --- 評価ループ ---
    student_model.eval()
    data_num = len(valid_images)
    indices = torch.randperm(data_num, device=device)
    correct = total = 0
    with torch.no_grad():
        for i in range(0, data_num, args.batch):
            idx = indices[i:i + args.batch]
            images = valid_images[idx]
...
def run(args):    
    ...
    data = CIFAR10GPUData(device=device)
    data.load()
    train_images, train_labels = data.get_train_data()
    valid_images, valid_labels = data.get_valid_data()

    # イテレータを作成して，１バッチ目を取得
    batch_gpu = train_images[: args.screen_batch].to(device)
    ...
    for gen in tqdm(range(1, args.generations+1), 
        ...
        if gen == 1:
            ...
            while len(teacher_hashes) < args.teacher_pool:
                ...
                data_num = len(train_images)  # データセットの長さを取得（後で計測に使う）
